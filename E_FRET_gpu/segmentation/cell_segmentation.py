...
from E_FRET_gpu.image.tool import normalize_image
"""
...
class SegmentationModel:
    ...
    def start(self, add_seg_channel=None, channels=None):
        self.dataloader()
        for sub_folder_path in self.matching_sub_folder_paths:
            self.process(sub_folder_path,
                         add_seg_channel=add_seg_channel,
                         channels=channels)

        self.remove_subdir_path()

    def process(self, current_image_set_path, add_seg_channel=None, channels=None):
        """
        ...
        :param channels:
        """
        print("分割处理 ===> ", current_image_set_path)
        if channels is None:
            channels = [0, 0, 0]
        min_size = self.min_box * self.min_box
        max_size = self.max_box * self.max_box
        ...
        combined_image = np.stack((normalize_image(image1), normalize_image(image2), normalize_image(image3)), axis=-1)
        ...
        self.mask_image(combined_image, diameter=self.diameter, channels=channels)
        ...
        if cell_sum >= 1:
            for value in range(1, cell_sum + 1):
                ...
                if len(indices[0]) < min_size or len(indices[0]) > max_size:
                    continue
    ...
    def mask_image(self, img, diameter, channels):
        if channels is None:
            channels = [0, 0, 0]
        self.current_img = img
        img = self.gaussian()
...
if __name__ == "__main__":
    ...
    segmentationModel.process(r'D:\data\qrm\EGFR_CFP+GRB2_YFP\2024.06.11_A549_SN_8H\A549-GEF-8h\23')
